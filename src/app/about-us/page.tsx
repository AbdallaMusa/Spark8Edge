...
const ScrollytellingSection = memo(function ScrollytellingSection() {
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) => (prev - 1 + SCROLLYTELLING_STEPS.length) % SCROLLYTELLING_STEPS.length);
  };
      ...
      >
        ...
        <div className="relative w-full max-w-4xl mx-auto">
          <div className="absolute top-1/2 -left-4 md:-left-16 -translate-y-1/2 z-20">
            <button onClick={prevSlide} className="p-3 rounded-full bg-white hover:bg-[#DFA236] text-[#040F2D] transition-all shadow-lg border border-[#040F2D]/10">
              <ChevronLeft size={24} />
            </button>
          </div>
          <div className="absolute top-1/2 -right-4 md:-right-16 -translate-y-1/2 z-20">
            <button onClick={nextSlide} className="p-3 rounded-full bg-white hover:bg-[#DFA236] text-[#040F2D] transition-all shadow-lg border border-[#040F2D]/10">
...
const ArchitectsSection = memo(function ArchitectsSection() {
  ...
  return (
    <section className="h-screen snap-start w-full bg-[#040F2D] flex items-center justify-center px-6 overflow-hidden relative">
      <motion.div
        variants={staggerContainer}
        initial="hidden"
        whileInView="show"
          ...
          <motion.div variants={fadeInUp} className="lg:col-span-7 space-y-8">
            <div>
              ...
              <p className="font-inter text-lg text-gray-300 leading-relaxed">
                Vanessa Mwando founded Spark8Edge with a clear mission: to bridge the critical gap between Nairobi&apos;s unemployed youth and corporations seeking innovative talent. Her vision transforms potential into professional excellence.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white/5 rounded-xl p-6 border border-white/10 hover:border-[#DFA236]/50 transition-all group">
                ...
                <h4 className="font-montserrat font-bold text-lg text-white mb-2">For Youth</h4>
                <p className="font-inter text-sm text-gray-400">Free technical training & mentorship pathways to employment.</p>
              </div>

              <div className="bg-white/5 rounded-xl p-6 border border-white/10 hover:border-[#DFA236]/50 transition-all group">
                <div className="w-10 h-10 rounded-full bg-[#DFA236]/10 flex items-center justify-center mb-4 group-hover:bg-[#DFA236] transition-colors">
                  <Briefcase className="text-[#DFA236] group-hover:text-[#040F2D]" size={20} />
                </div>
                <h4 className="font-montserrat font-bold text-lg text-white mb-2">For Organizations</h4>
                <p className="font-inter text-sm text-gray-400">Access to pre-vetted, job-ready talent pools.</p>
              </div>
            </div>

            <blockquote className="pl-6 border-l-4 border-[#DFA236] italic text-gray-300 font-inter text-lg">
              &quot;Every unemployed youth represents untapped potential. Spark8Edge is the catalyst that transforms potential into success.&quot;
            </blockquote>
          </motion.div>
        </div>
      </motion.div>
...
const PartnersSection = memo(function PartnersSection() {
  return (
    <section className="h-screen snap-start w-full bg-[#F4F4F9] flex items-center justify-center px-6 overflow-hidden">
      <div
      ...
      >
        <div className="text-center max-w-3xl mx-auto mb-12">
          <motion.h2 variants={fadeInUp} className="font-montserrat font-extrabold text-3xl md:text-4xl lg:text-5xl text-[#040F2D] mb-6">
            Enterprise-Grade Training
          </motion.h2>
          <motion.p variants={fadeInUp} className="font-inter text-lg text-[#6D8299]">
            A multidisciplinary curriculum blending creative artistry, AI innovation, and technical engineering to build the workforce of tomorrow.
          </motion.p>
        </div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 h-full">
              ...
              <ul className="space-y-4">
                {[
                  ...
                  <li key={i} className="flex items-start gap-3 text-[#6D8299] font-inter text-sm md:text-base">
                    <CheckCircle2 className="text-[#DFA236] shrink-0 mt-0.5" size={18} />
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <motion.div variants={fadeInUp} className="lg:col-span-7">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {TECHNOLOGIES_DATA.slice(0, 12).map((tech) => (
                <div key={tech.name} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:border-[#DFA236] hover:shadow-md transition-all group">
                  <div className="font-montserrat font-bold text-[#040F2D] group-hover:text-[#DFA236] transition-colors">{tech.name}</div>
...
const DualCTASection = memo(function DualCTASection() {
  return (
    <section className="h-screen snap-start w-full flex flex-col">
