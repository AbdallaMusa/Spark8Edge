...
import { Twitter, Linkedin, Instagram, ArrowRight, Facebook, Music } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          <div className="md:col-span-4">
            <Link href="/" className="inline-block mb-4">
              <h3 className="font-montserrat font-bold text-2xl text-white">
                Spark8<span className="text-[#DFA236]">Edge</span>
              </h3>
            </Link>
            <p className="font-inter text-gray-400 text-sm leading-relaxed max-w-xs">
              Bridging the gap between ambitious youth seeking tech careers and
              organizations needing fresh talent.
            </p>
          </div>

          <div className="md:col-span-2">
            <h4 className="font-montserrat font-semibold text-sm uppercase tracking-wider text-gray-300 mb-4">
              Navigate
            </h4>
            <ul className="space-y-3">
              {navLinks.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="font-inter text-sm text-gray-400 hover:text-[#DFA236] transition-colors duration-300"
                  >
                    {link.name}
                  </Link>
                </li>
          ...
          <div className="md:col-span-4 bg-white/5 border border-white/10 rounded-lg p-6">
            <h4 className="font-montserrat font-semibold text-base text-white mb-3">
              Take the Next Step
            </h4>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 text-center md:text-left">
          <p className="font-inter text-xs text-gray-500">
            &copy; {year} Spark8Edge. All Rights Reserved.
          </p>
        </div>
      </div>
    </motion.footer>
